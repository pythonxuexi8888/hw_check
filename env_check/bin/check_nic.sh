...
nic_report_file="nic_report.csv"
[[ -f $nic_report_file ]] && rm -f $nic_report_file
touch $nic_report_file
...
#    mac_sh=$(ip link | grep -A1 "$eth_sh" | grep 'link/ether' | awk '{print $2}')
    ...
    # output
    echo "$eth_sh,$numa_node_sh,$interrupt_str,$vendor_sh,$version_sh" >> $nic_report_file
done

sed -i "1i ID,numa_node,interrupt_node,vendor,product_version" $nic_report_file
rm -f $tmpfile
